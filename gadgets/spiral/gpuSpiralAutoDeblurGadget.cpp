...
typedef cuNFFT_plan<_real,2> plan_type;
	  ...
	  GadgetContainerMessage< hoNDArray<float> > *m3)
  {
	...
	*(header->getObjectPtr()) = tmp;
	...
	hoNDArray<_real> cost(&image_dims);
	...
	if(flag > 0){
		...
		if(true){
			...
			for(float f = -fmax; f <= fmax; f += fmax*2./(L-1)){
				...
				std::cout << f <<std::endl;	
				for( int ch = 0; ch < num_coils; ch++) {
					for( int k = 0; k < Nints; k++) {
						for( int i = 0; i < samples_per_interleave_; i++) {
							samples_demod[(ch*samples_per_interleave_*Nints)+(k*samples_per_interleave_)+i] *= exp(I*2*M_PI*f*i*sample_time);
						}
					}
				}
				...
				for (int i = 0; i < image_dims[0]*image_dims[1]; i++) {
					...
					if( std::abs(f-output_map[i]) <= 50 && std::abs(std::sin(arg(temp_image[i]))) < std::abs(std::sin(arg(output_image[i]))) ){
