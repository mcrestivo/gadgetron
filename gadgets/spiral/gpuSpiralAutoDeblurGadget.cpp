#include "gpuSpiralAutoDeblurGadget.h"
...
typedef cuNFFT_plan<_real,2> plan_type;
  ...
  gpuSpiralAutoDeblurGadget::gpuSpiralAutoDeblurGadget()
  ...
  int gpuSpiralAutoDeblurGadget::process_config(ACE_Message_Block* mb)
  {
...
*/	
		//Compute Base Images
		//hoNDArray<_complext> output_image(&image_dims);
		output_image.fill(_complext(0,1));
		hoNDArray<_complext> temp_image(&image_dims);
		hoNDArray<float> temp_phase(&image_dims);
		hoNDArray<_complext> samples_demod( new hoNDArray<_complext> );
		int j = 0;
		L = fmax;
		//int indx = 0;
		//timer = new GPUTimer("MFI Deblurring");
		for(float f = -fmax; f <= fmax; f += fmax*2./(L-1)){
			temp_phase.fill(0.0f);
			...
			for (i = image_dims[0]*2+2; i < image_dims[0]*image_dims[1]-(image_dims[0]*2+2); i++) {
				//indx = output_map[i]+fmax;
				//output_image[i] += reinterpret_cast<std::complex<float>>(MFI_C[indx*L+j]*temp_image[i]);
				//cost[i] = std::abs(std::sin(std::arg(temp_image[i])));
				for(int W1 = -2; W1 < 3; W1++){
					for(int W2 = -2; W2 < 3; W2++){
						temp_phase[i] += std::abs(std::sin(arg(temp_image[i+W1*image_dims[0]+W2])));
					}
				}
				//std::cout << temp_phase[i] << std::endl;
				if( /*std::abs(f-output_map[i])<50 &&*/ temp_phase[i]/25 < std::abs(std::sin(arg(output_image[i]))) ){
					output_image[i] = temp_image[i];
				}
			}
			//delete timer;
			j++;
...
//  gpuSpiralAutoDeblurGadget::duplicate_profile( GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *profile )
//  {
...
//  }
  ...
  GADGET_FACTORY_DECLARE(gpuSpiralAutoDeblurGadget)
}
