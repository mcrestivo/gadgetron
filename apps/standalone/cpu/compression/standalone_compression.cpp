...
int main(int argc, char** argv)
{
	...
	for(int ii = 0; ii < L; ii++){
		...
		int32_t number_elements = 2*128;
		std::random_device generator;
		std::normal_distribution<float> distribution(0.0,100);
		...
		compressed_size = con.compress_zfp_tolerance(&data_in[0], number_elements/4., 4, local_tolerance, comp_buffer_zfp, comp_buffer_size);
		compression_ratio = sizeof(float)*float(number_elements)/float(compressed_size);
		std::cout << compression_ratio << std::endl;
		con.decompress_zfp(comp_buffer_zfp, &data_out_zfp[0], number_elements/4., 4, comp_buffer_size);
